...
export function parseAccessQuery(query: URLSearchParams) { exact(query, ["referenceDate", "subjectUserId", "subjectRepresentationId"]); const subjectUserId = query.get("subjectUserId") ?? undefined; const subjectRepresentationId = query.get("subjectRepresentationId") ?? undefined; if (Number(Boolean(subjectUserId)) + Number(Boolean(subjectRepresentationId)) > 1) throw new GovernedMemoryHttpValidationError("Only one access subject is allowed."); return { referenceDate: iso(required(query, "referenceDate"), "referenceDate"), subjectUserId: subjectUserId ? parseId(subjectUserId, "subjectUserId") : undefined, subjectRepresentationId: subjectRepresentationId ? parseId(subjectRepresentationId, "subjectRepresentationId") : undefined }; }
