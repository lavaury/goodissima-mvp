...
async function appendHumanEvent(tx: Tx, input: { relationCaseId: string; type: GovernedMemoryEventType; actorUserId: string; actorRepresentationId?: string | null; objectType: GovernedMemoryTargetType; objectId: string; occurredAt: Date; recordedAt: Date; summary: string }) {
  const relationCase = await tx.relationCase.findUnique({ where: { id: input.relationCaseId }, select: { templateId: true } });
  if (!relationCase?.templateId) throw new Error("GOVERNED_MEMORY_SCOPE_NOT_FOUND");
  return tx.governedMemoryEvent.create({ data: { ...input, relationTemplateId: relationCase.templateId, governedJourneyId: null, actorType: "HUMAN" }, select: { id: true } });
}

type ExistingSourceCreateInput = Omit<Prisma.GovernedMemorySourceUncheckedCreateInput, "relationTemplateId" | "relationCaseId"> & { relationCaseId: string };

async function resolveGovernedMemoryScope(tx: Tx, relationCaseId: string, governedJourneyId?: string | null) {
  const relationCase = await tx.relationCase.findUnique({ where: { id: relationCaseId }, select: { id: true, templateId: true } });
  if (!relationCase?.templateId) throw new Error("GOVERNED_MEMORY_SCOPE_NOT_FOUND");
  if (governedJourneyId) {
    const journey = await tx.governedJourney.findFirst({ where: { id: governedJourneyId, relationTemplateId: relationCase.templateId }, select: { id: true } });
    if (!journey) throw new Error("GOVERNED_MEMORY_SCOPE_MISMATCH");
  }
  return { relationTemplateId: relationCase.templateId, relationCaseId: relationCase.id, governedJourneyId: governedJourneyId ?? null };
}
  ...
  return {
    ...
    createProposedFact(input: { relationCaseId: string; statement: string; evidenceLevel: Prisma.GovernedMemoryFactCreateInput["evidenceLevel"]; authorUserId: string; authorRepresentationId?: string | null; recordedAt: Date; effectiveFrom: Date; effectiveUntil?: Date | null }) {
      return database.$transaction(async (tx) => {
        const scope = await resolveGovernedMemoryScope(tx, input.relationCaseId);
        const fact = await tx.governedMemoryFact.create({ data: { ...input, ...scope, status: "PROPOSED" }, select: factSelect });
    ...
    establishFactConditionally(input: { relationCaseId: string; id: string; expectedUpdatedAt: Date; validatorUserId: string; validatorRole: GovernedMemoryRole; rationale: string; now: Date }) {
      return database.$transaction(async (tx) => {
        ...
        await tx.governedMemoryValidation.create({ data: { ...scope, governedJourneyId: null, relationCaseId: input.relationCaseId, targetType: "FACT", targetId: input.id, validatorUserId: input.validatorUserId, validatorRole: input.validatorRole, decision: "APPROVED", rationale: input.rationale, validatedAt: input.now } });
    ...
    supersedeFactTransactionally(input: { relationCaseId: string; priorFactId: string; expectedUpdatedAt: Date; statement: string; evidenceLevel: Prisma.GovernedMemoryFactCreateInput["evidenceLevel"]; actorUserId: string; effectiveFrom: Date; now: Date }) {
      return database.$transaction(async (tx) => {
        ...
        const successor = await tx.governedMemoryFact.create({ data: { ...scope, statement: input.statement, status: "PROPOSED", evidenceLevel: input.evidenceLevel, authorUserId: input.actorUserId, recordedAt: input.now, effectiveFrom: input.effectiveFrom, supersedesFactId: prior.id }, select: factSelect });
    ...
    createDraftDecision(input: { relationCaseId: string; title: string; rationale: string; decidedByUserId: string; decidedAt: Date; recordedAt: Date; effectiveFrom: Date; effectiveUntil?: Date | null; consequences?: string | null; reservations?: string | null }) {
      return database.$transaction(async (tx) => { const scope = await resolveGovernedMemoryScope(tx, input.relationCaseId); const decision = await tx.governedMemoryDecision.create({ data: { ...input, ...scope, status: "DRAFT" }, select: decisionSelect }); await appendHumanEvent(tx, { relationCaseId: input.relationCaseId, type: "DECISION_RECORDED", actorUserId: input.decidedByUserId, objectType: "DECISION", objectId: decision.id, occurredAt: input.decidedAt, recordedAt: input.recordedAt, summary: "Decision draft recorded by a human actor." }); return decision; });
    },
    ...
    validateDecisionConditionally(input: { relationCaseId: string; id: string; expectedUpdatedAt: Date; validatorUserId: string; validatorRole: GovernedMemoryRole; rationale: string; now: Date }) {
      return database.$transaction(async (tx) => {
        const scope = await resolveGovernedMemoryScope(tx, input.relationCaseId);
        const changed = await tx.governedMemoryDecision.updateMany({ where: { id: input.id, relationCaseId: input.relationCaseId, status: "DRAFT", updatedAt: input.expectedUpdatedAt }, data: { status: "VALIDATED", rationale: input.rationale, validatedByUserId: input.validatorUserId, validatedAt: input.now } });
        if (changed.count !== 1) return null;
        await tx.governedMemoryValidation.create({ data: { ...scope, governedJourneyId: null, relationCaseId: input.relationCaseId, targetType: "DECISION", targetId: input.id, validatorUserId: input.validatorUserId, validatorRole: input.validatorRole, decision: "APPROVED", rationale: input.rationale, validatedAt: input.now } });
        await appendHumanEvent(tx, { relationCaseId: input.relationCaseId, type: "DECISION_VALIDATED", actorUserId: input.validatorUserId, objectType: "DECISION", objectId: input.id, occurredAt: input.now, recordedAt: input.now, summary: "Decision validated by an authorized human." });
        return tx.governedMemoryDecision.findUnique({ where: { id_relationCaseId: { id: input.id, relationCaseId: input.relationCaseId } }, select: decisionSelect });
      });
    },
    createSuccessorDecision(input: { relationCaseId: string; priorDecisionId: string; type: "REPLACES" | "CORRECTS" | "CANCELS" | "COMPLEMENTS"; title: string; rationale: string; actorUserId: string; effectiveFrom: Date; now: Date }) {
      return database.$transaction(async (tx) => { const scope = await resolveGovernedMemoryScope(tx, input.relationCaseId); const prior = await tx.governedMemoryDecision.findUnique({ where: { id_relationCaseId: { id: input.priorDecisionId, relationCaseId: input.relationCaseId } }, select: { id: true, status: true } }); if (!prior || prior.status === "DRAFT") return null; const successor = await tx.governedMemoryDecision.create({ data: { ...scope, title: input.title, rationale: input.rationale, status: "DRAFT", decidedByUserId: input.actorUserId, decidedAt: input.now, recordedAt: input.now, effectiveFrom: input.effectiveFrom }, select: decisionSelect }); await tx.governedMemoryRelation.create({ data: { relationCaseId: input.relationCaseId, type: input.type, sourceType: "DECISION", sourceId: successor.id, targetType: "DECISION", targetId: prior.id, createdByUserId: input.actorUserId } }); return successor; });
    },
    ...
    registerSource(input: ExistingSourceCreateInput, actorUserId: string, now: Date) {
      return database.$transaction(async (tx) => { const scope = await resolveGovernedMemoryScope(tx, input.relationCaseId, input.governedJourneyId); const source = await tx.governedMemorySource.create({ data: { ...input, ...scope }, select: sourceSelect }); await appendHumanEvent(tx, { relationCaseId: input.relationCaseId, type: source.kind === "MESSAGE_EXCERPT" ? "SOURCE_PROMOTED" : "SOURCE_REGISTERED", actorUserId, objectType: "SOURCE", objectId: source.id, occurredAt: source.promotedAt ?? source.recordedAt, recordedAt: now, summary: source.kind === "MESSAGE_EXCERPT" ? "Private message excerpt promoted explicitly." : "Memory source registered explicitly." }); return source; });
    },
    ...
    appendValidation(input: Omit<Prisma.GovernedMemoryValidationUncheckedCreateInput, "relationTemplateId" | "governedJourneyId"> & { relationCaseId: string }) { return database.$transaction(async (tx) => { if (!await targetExists(tx, input.relationCaseId, input.targetType, input.targetId)) return null; const scope = await resolveGovernedMemoryScope(tx, input.relationCaseId); const validation = await tx.governedMemoryValidation.create({ data: { ...input, ...scope, governedJourneyId: null } }); await appendHumanEvent(tx, { relationCaseId: input.relationCaseId, type: "VALIDATION_RECORDED", actorUserId: input.validatorUserId, objectType: input.targetType, objectId: input.targetId, occurredAt: new Date(input.validatedAt), recordedAt: validation.createdAt, summary: "Append-only human validation recorded." }); return validation; }); },
    openDispute(input: Omit<Prisma.GovernedMemoryDisputeUncheckedCreateInput, "relationTemplateId" | "governedJourneyId"> & { relationCaseId: string }) { return database.$transaction(async (tx) => { if (!await targetExists(tx, input.relationCaseId, input.targetType, input.targetId)) return null; const scope = await resolveGovernedMemoryScope(tx, input.relationCaseId); const dispute = await tx.governedMemoryDispute.create({ data: { ...input, ...scope, governedJourneyId: null } }); await appendHumanEvent(tx, { relationCaseId: input.relationCaseId, type: "DISPUTE_OPENED", actorUserId: input.raisedByUserId, objectType: input.targetType, objectId: input.targetId, occurredAt: new Date(input.raisedAt), recordedAt: dispute.createdAt, summary: "Memory object disputed without deleting its history." }); return dispute; }); },
    ...
    appendEvent(input: Omit<Prisma.GovernedMemoryEventUncheckedCreateInput, "relationTemplateId" | "governedJourneyId"> & { relationCaseId: string }) { return database.$transaction(async (tx) => { if (!await targetExists(tx, input.relationCaseId, input.objectType, input.objectId)) return null; const relationCase = await tx.relationCase.findUnique({ where: { id: input.relationCaseId }, select: { templateId: true } }); if (!relationCase?.templateId) return null; return tx.governedMemoryEvent.create({ data: { ...input, relationTemplateId: relationCase.templateId, governedJourneyId: null } }); }); },
