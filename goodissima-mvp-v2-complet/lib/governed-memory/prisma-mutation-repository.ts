...
export class PrismaGovernedMemoryMutationRepository implements GovernedMemoryMutationRepository {
  async execute(userId: string, input: MemoryMutationInput, fingerprint: string, ids: { request: string; object: string; event: string; auxiliary: string }, now: Date): Promise<MemoryMutationResult> {
    return prisma.$transaction(async (tx) => {
      ...
      const journey = await tx.governedJourney.findUnique({ where: { id: input.journeyId }, select: { id: true, relationTemplateId: true, authorityUserId: true, relationCaseContexts: { where: input.relationCaseId ? { relationCaseId: input.relationCaseId } : undefined, select: { relationCaseId: true } }, memoryRoleAssignments: { where: { userId, revokedAt: null }, select: { role: true } } } });
      ...
      if (!grant && !role && !isJourneyAuthority) throw new Error("MEMORY_NOT_FOUND");
      ...
      if (input.operation === "ESTABLISH_FACT") { const fact = await tx.governedMemoryFact.findFirst({ where: { id: input.targetId, ...base } }); if (!fact || fact.status !== "PROPOSED" || (!role && !isJourneyAuthority)) throw new Error("MEMORY_NOT_FOUND"); objectId = fact.id; validationId = role ? ids.auxiliary : null; await tx.governedMemoryFact.update({ where: { id: fact.id }, data: { status: "ESTABLISHED", establishedByUserId: userId, establishedAt: now, updatedAt: now } }); if (role) await tx.governedMemoryValidation.create({ data: { id: validationId!, targetType: "FACT", targetId: fact.id, validatorUserId: userId, validatorRole: role.role, decision: "APPROVED", validatedAt: now, ...base } }); }
      if (input.operation === "VALIDATE_DECISION") { const decision = await tx.governedMemoryDecision.findFirst({ where: { id: input.targetId, ...base } }); if (!decision || decision.status !== "DRAFT" || (!role && !isJourneyAuthority)) throw new Error("MEMORY_NOT_FOUND"); objectId = decision.id; validationId = role ? ids.auxiliary : null; await tx.governedMemoryDecision.update({ where: { id: decision.id }, data: { status: "VALIDATED", validatedByUserId: userId, validatedAt: now, updatedAt: now } }); if (role) await tx.governedMemoryValidation.create({ data: { id: validationId!, targetType: "DECISION", targetId: decision.id, validatorUserId: userId, validatorRole: role.role, decision: "APPROVED", validatedAt: now, ...base } }); }
