...
export class PrismaGLinkMatchingSourceStore implements GLinkMatchingSourceStore {
  ...
  async listStructuredCandidatesForOwner(ownerId: string, excludedGLinkId: string, oppositeType: "OFFER" | "NEED", limit: number) {
    const links = await this.client.gLink.findMany({
      where: { ownerId, status: "ACTIVE", id: { not: excludedGLinkId }, AND: [{ rules: { path: ["opportunity", "type"], equals: oppositeType } }, { rules: { path: ["opportunity", "matchingEnabled"], equals: true } }] },
      orderBy: { id: "asc" }, take: limit,
      select: { id: true, ownerId: true, status: true, templateId: true, rules: true },
    });
    return links.map((link) => ({ sourceType: "GLINK" as const, sourceId: link.id, ownerId: link.ownerId, title: "", description: null, fields: [], status: link.status, rules: link.rules, templateId: link.templateId }));
  }
}
