...
R5-I2a ajoute la racine `RelationTemplate` au journal mémoire, rend ses scopes dossier et parcours facultatifs et impose qu'au moins l'un soit présent. Le backfill est strictement structurel depuis les dossiers historiques; aucun `GovernedJourney` n'est inféré. `REGISTER_SOURCE` est attribué uniquement aux rôles `RELATION_CASE_OWNER`, `MEMORY_STEWARD` et `MEMORY_DELEGATE`; à cette étape, `VIEW_SOURCES` restait distinct et le contrôle historique devait encore être remplacé en R5-I2b. Aucune commande journey-scoped, idempotence branchée ou UI n'était livrée en R5-I2a.

R5-I2b active `REGISTER_SOURCE` pour l'écriture ordinaire et branche `GovernedMemoryCreationRequest` sur les trois commandes journey-scoped. La réservation est la première écriture, la complétion la dernière, et objet plus événement mémoire sont atomiques entre les deux. La récupération compare fingerprint, catégorie et scopes après un lookup exclusivement demandeur+clé. R4 lit aussi les rattachements directs et déduplique les chemins historiques. Aucune UI ou Server Action cockpit n'est ajoutée; R5-II reste le point d'entrée humain visible.
