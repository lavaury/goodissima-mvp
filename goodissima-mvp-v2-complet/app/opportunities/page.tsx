...
export default async function OpportunitiesPage({ searchParams }: { searchParams?: { templateId?: string } }) {
  const owner = await getCurrentPrismaUser();
  const organizationName = owner.name && owner.name !== owner.email ? owner.name : "Organisation Goodissima";
  const announcements = await prisma.gLink.findMany({
    where: { ownerId: owner.id, ...(searchParams?.templateId ? { templateId: searchParams.templateId } : {}) },
  ...
  return <main className="mx-auto max-w-6xl px-6 py-10">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#247f88]">Mes opportunités et relations</p><h1 className="mt-2 text-3xl font-bold">Opportunités</h1><ProductObjectDefinition object="announcement" /></div><div className="flex flex-wrap gap-2"><Link href="/opportunities/new" className="rounded-xl bg-slate-900 px-4 py-2 text-sm font-semibold text-white">Créer une opportunité</Link><LogoutButton /></div></div>
    ...
    <div className="mt-6">{announcements.length ? <div className="grid gap-5 lg:grid-cols-2">{announcements.map((item) => <LinkCard key={item.id} item={{ id: item.id, slug: item.slug, title: item.title, city: item.city, templateName: item.template?.name, templateStatus: item.template?.status, templateVersion: item.templateVersion?.version, sourceJourneyHref: item.template?.formTemplates[0] ? `/templates/${item.template.formTemplates[0].id}` : undefined, cases: item.cases }} />)}</div> : <div className="rounded-3xl border border-dashed bg-white p-10 text-center"><h2 className="font-semibold">Aucune annonce pour le moment</h2><p className="mt-2 text-sm text-slate-500">Créez une annonce à partir d'un parcours validé.</p></div>}</div>
  </main>;
}
