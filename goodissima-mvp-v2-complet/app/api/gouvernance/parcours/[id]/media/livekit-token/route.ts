...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const preferred = preferredSessionId ? await prisma.communicationSession.findFirst({ where: { id: preferredSessionId, ownerId: owner.id, relationTemplateId: form.relationTemplate.id, relationCaseId: null, status: { in: ["REQUESTED", "PREPARED_NOT_STARTED"] } } }) : null;
