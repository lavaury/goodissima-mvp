...
} from "@/lib/governed-journey-consent-actions";
import {
  hasCurrentJourneyAccess,
  projectJourneyConsent,
} from "@/lib/governed-journey-consent";
...
export default async function GuestJourneyPage({
  ...
  const invitation = await prisma.governedJourneyInvitation.findUnique({
    ...
    include: {
      consent: true,
      relationTemplate: { select: { name: true, description: true } },
    },
  });
  ...
  if (
    ...
    invitation.accessTokenExpiresAt <= new Date()
  ) {
    return (
      <main className="mx-auto max-w-2xl p-8">
        <h1 className="text-2xl font-bold">Accès refusé</h1>
        <p className="mt-3">Ce lien est expiré ou a été révoqué.</p>
        <p className="mt-3">
          <strong>Invité par :</strong>{" "}
          {inviter?.name || "L’organisateur du parcours"}
        </p>
      </main>
    );
  }
  ...
  const currentUser = authUser?.email
    ? await prisma.user.findUnique({
        where: { email: normalizeInvitationEmail(authUser.email) },
        select: { id: true },
      })
    : null;
  const consentFlow = projectJourneyConsent(invitation) === "NEW_CONSENT_FLOW";
  const participationContext = getGovernedInvitationRoleLabel(
    invitation.role,
    invitation.metadata,
  );
    ...
    return (
      <main className="mx-auto max-w-2xl p-8">
        ...
        <p className="mt-3 text-slate-600">
          Aucun accès au parcours ou à ses réunions n’a été ouvert.
        </p>
      </main>
    );
  }
  ...
  if (
    ...
    invitation.inviteeUserId !== currentUser.id
  ) {
    return (
      <main className="mx-auto max-w-2xl p-8">
        <p className="text-sm font-semibold text-[#247f88]">
        ...
        <h1 className="mt-2 text-2xl font-bold">Invitation indisponible</h1>
        <p className="mt-3">Cette invitation est liée à un autre compte.</p>
      </main>
    );
  }
  ...
  if (
    ...
    return (
      <main className="mx-auto max-w-2xl p-8">
        ...
        <h1 className="mt-2 text-2xl font-bold">Identité requise</h1>
        <Link
          className="mt-6 inline-block rounded bg-[#247f88] px-5 py-3 font-semibold text-white"
          href={`/login?next=${encodeURIComponent(`/gouvernance/invitation/${params.token}`)}`}
        >
          Se connecter pour accéder au parcours
        </Link>
      </main>
    );
  }
  ...
  if (consentFlow && !hasCurrentJourneyAccess(invitation)) {
    const canDecide = invitation.inviteeUserId
      ? invitation.inviteeUserId === currentUser?.id
      : true;
    ...
    return (
      <main className="mx-auto max-w-2xl p-8">
        ...
        <h1 className="mt-2 text-3xl font-bold">
          ...
          {invitation.relationTemplate.name} »
        </h1>
        <p className="mt-4">
          <strong>Objectif :</strong>{" "}
          {invitation.relationTemplate.description ||
            "Participer à ce parcours gouverné."}
        </p>
        <section className="mt-6 rounded-lg border bg-slate-50 p-5">
          <h2 className="font-bold">Votre participation</h2>
          <p className="mt-2">
            <strong>Invité par :</strong>{" "}
            {inviter?.name || "L’organisateur du parcours"}
          </p>
          {participationContext ? (
            <p className="mt-2">
              <strong>Contexte de participation :</strong>{" "}
              {participationContext}
            </p>
          ) : (
            <p className="mt-2">Participation prévue</p>
          )}
          <p className="mt-3 text-sm">
            Vous pourrez consulter les éléments partagés dans ce parcours et
            contribuer selon le rôle proposé. Les réunions nécessitent également
            une autorisation propre.
          </p>
          {!invitation.inviteeUserId ? (
            <p className="mt-3 rounded border border-amber-200 bg-amber-50 p-3 text-sm text-amber-950">
              Cette invitation personnelle prouve la possession du lien, mais ne
              vérifie pas votre identité. Le nom affiché a été déclaré par
              l’organisateur.
            </p>
          ) : null}
        </section>
        {canDecide ? (
          <div className="mt-6 flex flex-wrap gap-3">
            <form action={acceptJourneyInvitation}>
              <input
                type="hidden"
                name="invitationToken"
                value={params.token}
              />
              <button className="rounded bg-[#247f88] px-5 py-3 font-semibold text-white">
                Accepter de participer
              </button>
            </form>
            <form action={declineJourneyInvitation}>
              <input
                type="hidden"
                name="invitationToken"
                value={params.token}
          ...
          <p className="mt-6 rounded border border-amber-200 bg-amber-50 p-4 text-sm">
            {invitation.inviteeUserId
              ? "Cette invitation est liée à un autre compte."
              : "La réponse en ligne n’est pas encore disponible pour une invitation externe sans identité vérifiée."}
          </p>
        )}
        {searchParams?.decision === "unavailable" ? (
          <p className="mt-4 text-sm text-red-700">
            Cette décision ne peut pas être enregistrée.
          </p>
        ) : null}
      </main>
    );
  ...
  const meetings = await prisma.governedMeetingParticipant.findMany({
    ...
    include: { communicationSession: true, rsvp: true },
  ...
  return (
    <main className="mx-auto max-w-2xl p-8">
      ...
      <h1 className="mt-2 text-3xl font-bold">
        Vous participez à « {invitation.relationTemplate.name} »
      </h1>
      <p className="mt-4">
        <strong>Objectif :</strong> {invitation.relationTemplate.description}
      </p>
      <section className="mt-6 rounded-lg border bg-slate-50 p-5">
        <h2 className="font-bold">Votre participation</h2>
        <p className="mt-2">
          <strong>Invitation envoyée par :</strong>{" "}
          {inviter?.name || "L’organisateur du parcours"}
        </p>
        <p className="mt-2">
          <strong>Personne invitée :</strong> {invitation.displayName}
        </p>
        {participationContext ? (
          <p>
            <strong>Contexte de participation :</strong> {participationContext}
          </p>
        ) : null}
        <p className="mt-3 text-sm">
          Votre accès est limité à ce parcours. Cette invitation ne vous donne
          pas automatiquement accès à toutes ses réunions, à un autre dossier ou
          à l’espace de l’organisateur.
        </p>
        {!invitation.inviteeUserId ? (
          <p className="mt-3 text-sm font-semibold text-amber-900">
            L’invitation personnelle a été acceptée. Elle ne constitue pas une
            identité vérifiée.
          </p>
        ) : null}
      </section>
                  ...
                  <div className="mt-4">
                    ...
                    <div className="flex gap-3">
                      <form action={acceptMeetingRsvp}>
                        ...
                        />
                        <button className="rounded bg-[#247f88] px-4 py-2 font-semibold text-white">
                          Participer
                        </button>
                      </form>
                      <form action={declineMeetingRsvp}>
                        <input
                          type="hidden"
                          name="invitationToken"
                          value={params.token}
                ...
                {live ? (
                  <>
                    <p className="my-3 text-sm text-slate-600">
                      Audio, vidéo et partage d’écran sont disponibles dans une
                      salle dédiée.
                    </p>
                    <Link
                      href={`/gouvernance/invitation/${params.token}/reunions/${session.id}`}
                      className="inline-flex min-h-11 items-center rounded bg-[#247f88] px-4 py-2 font-semibold text-white"
                    >
                      Rejoindre la salle
                    </Link>
                  </>
