...
export default async function RelationsPage() {
  ...
  const [relations, relationRequests] = await Promise.all([
    ...
    prisma.publicCaseCreationRequest.findMany({ where: { status: "PENDING", gLink: { ownerId: owner.id }, requestPayload: { not: Prisma.DbNull } }, orderBy: { createdAt: "desc" }, include: { gLink: { select: { id: true, title: true, rules: true, templateId: true } } } }),
  ]);
  ...
  const requests = relationRequests.flatMap((request) => { const data = request.requestPayload as Record<string, unknown>; if (typeof data.candidateName !== "string" || typeof data.candidateEmail !== "string") return []; return [{ id: request.id, gLinkId: request.gLinkId, title: request.gLink.title, candidateName: data.candidateName, createdAt: request.createdAt, href: relationRequestOwnerHref(request.gLink, request.id) }]; });
  return <main className="mx-auto max-w-6xl px-6 py-10">
    ...
    <ProductLifecycle current="relation" />{requests.length ? <section className="mt-6 rounded-3xl border bg-white p-6"><h2 className="text-xl font-bold">Demandes de relation</h2><p className="mt-2 text-sm text-slate-600">{requests.length} demande{requests.length > 1 ? "s" : ""} attend{requests.length > 1 ? "ent" : ""} votre décision.</p><div className="mt-4 space-y-3">{requests.map((request) => <article key={request.id} className="rounded-2xl border p-4"><h3 className="font-semibold">Demande de relation</h3><p className="mt-1 text-sm">Lien : {request.title}</p><p className="mt-1 text-sm text-slate-600">{request.candidateName || "Demandeur"} · {request.createdAt.toLocaleDateString("fr-FR")}</p><Link href={request.href} className="mt-3 inline-flex min-h-11 items-center rounded-xl border px-4 text-sm font-semibold">Examiner la demande</Link></article>)}</div></section> : null}
