...
export default async function TemplateDetailPage({ params, searchParams }: { params: { templateId: string }; searchParams?: { created?: string; assistant?: string; advanced?: string } }) {
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-10">
      <PageNavigationContext pathname={`/templates/${encodeURIComponent(template.id)}`} items={objectBreadcrumb({ name: localizeTemplateName(template.relationTemplate?.key, template.name, locale), fallback: "Parcours", objectId: template.id, ownerId: owner.id, workspace: template.relationTemplate?.workspace })} />
