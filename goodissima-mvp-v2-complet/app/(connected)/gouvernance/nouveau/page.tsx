...
function WorkspaceFields({ workspaces, initialWorkspaceId = "" }: { workspaces: GovernanceWorkspaceOption[]; initialWorkspaceId?: string }) {
  return (
    ...
    <main className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="mt-6">
        <Link href={workspace ? `/gouvernance/workspaces/${encodeURIComponent(workspace.id)}` : "/gouvernance"} className="text-sm font-semibold text-slate-600 underline underline-offset-4">
          {workspace ? `Retour à ${businessLabel(workspace.name, "ce Workspace", [workspace.id])}` : "Retour a la gouvernance"}
        </Link>
      </div>
      ...
      <GovernanceJourneyAssistant key={workspace?.id ?? "generic"} workspaces={workspaces} initialWorkspaceId={workspace?.id} />
      ...
      <form action={createGovernedJourneyAction} data-boussole-id="manual-governed-journey-form" className="mt-6 space-y-5 rounded-lg border bg-white p-6 shadow-sm">
        ...
        <WorkspaceFields key={workspace?.id ?? "generic"} workspaces={workspaces} initialWorkspaceId={workspace?.id} />
