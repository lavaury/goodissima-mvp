...
export function PublicResponseContext({
  ...
  return <aside className="rounded-2xl border border-cyan-100 bg-cyan-50/60 p-5" aria-label="Contexte de votre réponse">
    ...
    {kind === "OPPORTUNITY" ? <a href="#public-link-context" onClick={(event) => { event.preventDefault(); const target = document.getElementById("public-link-context"); target?.scrollIntoView({ behavior: "smooth", block: "start" }); target?.focus({ preventScroll: true }); }} className="mt-4 inline-block text-sm font-semibold text-cyan-900 underline underline-offset-2">↑ Voir l’annonce complète</a> : null}
  </aside>;
}
