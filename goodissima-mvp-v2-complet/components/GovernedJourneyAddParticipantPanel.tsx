...
export function GovernedJourneyAddParticipantPanel({ formTemplateId, journeyTitle }: { formTemplateId: string; journeyTitle: string }) {
  ...
  return <details className="mt-4 rounded-lg border border-[#247f88]/40 bg-white p-4">
    ...
    {link ? <div className="mt-4 rounded-lg border border-emerald-300 bg-emerald-50 p-3"><input ref={linkRef} readOnly value={link} onFocus={(event) => event.currentTarget.select()} aria-label="Lien personnel d’invitation" className="w-full rounded border bg-white px-3 py-2 text-sm" /><button type="button" onClick={() => void copyLink()} className="mt-2 min-h-11 rounded-lg bg-emerald-800 px-4 py-2 font-bold text-white">Copier le lien</button></div> : null}
