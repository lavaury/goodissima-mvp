...
export function WelcomeEntryScene({ selectedKey, ...props }: SceneProps & { selectedKey: WelcomeEntryKey | null }) {
  return <SceneFrame {...props} shortDescription="Votre intention reste le point de départ de chaque possibilité." text="Une intention peut conduire vers quatre manières de commencer. La sélection reste humaine et n’ouvre aucune page."><div className={`${styles.animatedItem} mx-auto max-w-sm rounded-full border border-cyan-300 bg-white px-4 py-3 text-center text-base font-bold text-cyan-950`}>Ce que je souhaite accomplir</div><div className="mt-5 grid gap-3 sm:grid-cols-2">{welcomeEntries.map((entry, index) => <div key={entry.key} className={`${styles.card} ${styles.animatedItem} ${selectedKey && selectedKey !== entry.key ? styles.secondary : ""} ${selectedKey === entry.key ? styles.selected : ""}`} style={{ "--scene-delay": `${180 + index * 120}ms` } as React.CSSProperties}>{entry.title}</div>)}</div></SceneFrame>;
}
