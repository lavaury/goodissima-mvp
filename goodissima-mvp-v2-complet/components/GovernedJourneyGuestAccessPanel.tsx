...
type Access = { id: string; displayName: string; role: string; status: string; expiresAt: string };

export function GovernedJourneyGuestAccessPanel({ formTemplateId, participantName, participantRole, governedRole, preparedEmail, invitations }: {
  formTemplateId: string; participantName: string; participantRole: string; governedRole: string;
  preparedEmail?: string | null; invitations: Access[];
}) {
  ...
  async function createAccess(formData: FormData) {
    ...
    const response = await fetch("/api/gouvernance/invitations", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({
      ...
      role: governedRole, expiresInDays: formData.get("expiresInDays"),
    }) });
