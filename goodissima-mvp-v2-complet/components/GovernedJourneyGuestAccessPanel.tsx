...
export function GovernedJourneyGuestAccessPanel({ formTemplateId, participantName, participantRole, governedRole, preparedEmail, invitations, relationCases }: {
  ...
  return <section data-boussole-id="governed-journey-guest-access" data-boussole-state={active ? "active" : "to-create"} className="mt-4 rounded-lg border-2 border-[#247f88]/30 bg-white p-4">
    <h3 className="font-bold text-slate-950">Accès au parcours</h3>
    <p className="mt-1 text-sm text-slate-600">Créez un accès personnel limité à ce parcours. Chaque participant reçoit son propre lien personnel ; ce lien ne doit pas être partagé avec un autre participant et Goodissima ne l’envoie pas automatiquement. Les accès aux réunions sont choisis séparément.</p>
    ...
    {active ? <div className="mt-3 rounded-lg border border-emerald-200 bg-emerald-50 p-3">
      ...
      <p className="mt-1 text-xs text-emerald-900">Rôle : {participantRole} · Expiration : {new Date(active.expiresAt).toLocaleString("fr-FR")}</p>
      <p className="mt-2 text-xs text-emerald-800">{active.relationCaseId ? "Cet accès inclut le dossier sélectionné." : "Cet accès est limité au parcours."}</p>
      ...
      <label className="text-xs font-semibold text-slate-600">Dossier accessible avec ce parcours<select name="relationCaseId" className="mt-1 block min-h-11 max-w-xs rounded-lg border px-3 py-2 text-sm font-normal"><option value="">Parcours uniquement</option>{relationCases.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label>
      ...
      <button data-boussole-id="create-governed-invitation" className="min-h-11 rounded-lg bg-[#247f88] px-4 py-2 text-sm font-bold text-white">Créer l’accès au parcours</button>
    </form>}
