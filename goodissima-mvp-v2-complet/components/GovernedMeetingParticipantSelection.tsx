    ...
    <section data-boussole-id="governed-meeting-participant-selection" data-boussole-state={boussoleState} className="mt-4 rounded-lg border border-cyan-200 bg-cyan-50/70 p-3">
      ...
      {selection && (selection.status === "DRAFT" || selection.status === "UNDER_REVIEW") ? (
        <div className="mt-4 border-t border-cyan-200 pt-4">
          ...
          {selection.status === "UNDER_REVIEW" ? <div className="mt-4 rounded-lg bg-white p-3 text-sm text-slate-800"><p><strong>Source :</strong> Membres du Parcours</p><p>{formatMeetingSelectionResultCount(items.length, "candidat observé", "candidats observés")} · {formatMeetingSelectionResultCount(retained.length, "personne retenue", "personnes retenues")} · {formatMeetingSelectionResultCount(excluded, "exclu", "exclus")}</p><p>Parmi les retenus : {formatMeetingSelectionResultCount(toAdd, "à ajouter", "à ajouter")} · {formatMeetingSelectionResultCount(alreadyPresent, "déjà présent", "déjà présents")}</p></div> : null}
