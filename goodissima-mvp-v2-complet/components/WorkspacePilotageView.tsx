...
export function WorkspacePilotageView({ data, counts }: { data: WorkspacePilotage; counts: { journeys: number; links: number; cases: number } }) {
  return <div className="mt-6 min-w-0 space-y-6">
    <section data-boussole-id="workspace-volumes" aria-labelledby="workspace-volumes"><h2 id="workspace-volumes" className="text-xl font-bold">Objets de cet espace</h2>
    ...
    <section id="attention" data-boussole-id="workspace-attention" data-boussole-state={data.signals.length ? "populated" : "empty"} aria-labelledby="workspace-attention"><h2 id="workspace-attention" className="text-xl font-bold">À examiner</h2>
    ...
    <Communications boussoleId="workspace-recent" title="Communications récentes" empty="Aucune communication récente." items={data.recent} />
  </div>;
}

function Communications({ boussoleId, title, empty, items, upcoming = false }: { boussoleId: string; title: string; empty: string; items: WorkspacePilotage["recent"]; upcoming?: boolean }) {
  return <section data-boussole-id={boussoleId} data-boussole-state={items.length ? "populated" : "empty"} aria-label={title}><h2 className="text-xl font-bold">{title}</h2><p className="mt-2 text-sm text-slate-600">{upcoming ? "Les 10 prochaines réunions au maximum." : "Les 10 dernières communications mises à jour au cours des 14 derniers jours."} Rattachement enregistré dans cet espace.</p>
