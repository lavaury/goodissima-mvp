...
import { WelcomeEntryScene, WelcomeHandoffScene, WelcomePrincipleScene, WelcomeSituationScene, type WelcomeAnimationState } from "@/components/boussole/welcome/WelcomeScenes";
...
function EntryCards({ headingLevel, selectable, selectedKey, onSelect }: { headingLevel: 3 | 4; selectable: boolean; selectedKey: WelcomeEntryKey | null; onSelect: (entry: WelcomeEntry) => void }) {
  ...
  return <div className="mt-3 grid gap-4 md:grid-cols-2">{welcomeEntries.map((entry) => <article key={entry.key} data-boussole-id={entry.stableId} className={`flex flex-col rounded-xl border p-4 ${selectedKey === entry.key ? "border-cyan-700 bg-cyan-50 ring-2 ring-cyan-700" : "border-slate-200 bg-white"}`}><Heading className="font-bold text-slate-950">{entry.title}</Heading><p className="mt-2 text-sm leading-relaxed text-slate-600">{entry.description}</p>{entry.explanationOfGoodissimaTerm ? <p className="mt-2 text-sm font-semibold text-cyan-900">{entry.explanationOfGoodissimaTerm}</p> : null}<p className="mt-3 text-sm leading-relaxed text-slate-700"><strong>Situation type :</strong> {entry.situationType}</p><ol className="mt-4 grid gap-2 text-sm text-slate-700 sm:grid-cols-4">{entry.compactFlow.map((step, index) => <li key={step} className="flex gap-2 sm:flex-col"><span aria-hidden="true" className="font-bold text-cyan-800">{index + 1}.</span><span>{step}</span></li>)}</ol><p className="mt-4 rounded-lg bg-amber-50 p-3 text-sm leading-relaxed text-amber-950">{entry.humanControlNotice}</p><details className="mt-3"><summary className="min-h-11 cursor-pointer py-2 text-sm font-bold text-cyan-950 outline-none focus-visible:ring-2 focus-visible:ring-cyan-700">Voir un exemple détaillé</summary><p className="mt-2 text-sm leading-relaxed text-slate-700">{entry.detailedExample}</p></details>{selectable ? <button type="button" aria-pressed={selectedKey === entry.key} onClick={() => onSelect(entry)} className="mt-auto min-h-11 w-full rounded-lg border border-cyan-300 bg-white px-4 py-2 text-sm font-bold text-cyan-950 outline-none focus-visible:ring-2 focus-visible:ring-cyan-700 focus-visible:ring-offset-2 sm:w-auto">{selectedKey === entry.key ? `Possibilité sélectionnée : ${entry.title}` : `Choisir : ${entry.title}`}</button> : null}</article>)}</div>;
}
