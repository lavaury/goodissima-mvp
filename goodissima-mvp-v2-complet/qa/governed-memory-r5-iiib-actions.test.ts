...
test("thin transition actions authenticate, parse, delegate and revalidate R4", () => {
  for (const name of ["establishJourneyFactAction", "disputeJourneyFactAction", "validateJourneyDecisionAction", "revokeJourneyMemoryRoleAction"]) assert.match(actions, new RegExp(`export async function ${name}`));
...
test("public action states expose bounded French feedback without technical results", () => {
  assert.match(actionState, /GovernedMemoryTransitionActionState/);
