...
try {
  await prisma.$transaction(async (tx) => {
    ...
    await tx.$executeRawUnsafe(`CREATE FUNCTION pg_temp.rsvp_invalid(kind text, participant_id text, user_id text) RETURNS boolean LANGUAGE plpgsql AS $fn$ BEGIN BEGIN IF kind='pending_decided' THEN INSERT INTO "GovernedMeetingRsvp" ("id","meetingParticipantId","status","decidedAt","version","meetingRevision","createdAt","updatedAt") VALUES (gen_random_uuid()::text,participant_id,'PENDING',now(),0,1,now(),now()); ELSIF kind='accepted_no_date' THEN INSERT INTO "GovernedMeetingRsvp" ("id","meetingParticipantId","status","decidedByUserId","version","meetingRevision","createdAt","updatedAt") VALUES (gen_random_uuid()::text,participant_id,'ACCEPTED',user_id,0,1,now(),now()); ELSIF kind='declined_no_date' THEN INSERT INTO "GovernedMeetingRsvp" ("id","meetingParticipantId","status","decidedByUserId","version","meetingRevision","createdAt","updatedAt") VALUES (gen_random_uuid()::text,participant_id,'DECLINED',user_id,0,1,now(),now()); ELSIF kind='duplicate' THEN INSERT INTO "GovernedMeetingRsvp" ("id","meetingParticipantId","status","version","meetingRevision","createdAt","updatedAt") VALUES (gen_random_uuid()::text,participant_id,'PENDING',0,1,now(),now()); ELSIF kind='bad_revision' THEN INSERT INTO "GovernedMeetingRsvp" ("id","meetingParticipantId","status","version","meetingRevision","createdAt","updatedAt") VALUES (gen_random_uuid()::text,participant_id,'PENDING',0,0,now(),now()); ELSIF kind='bad_participant' THEN INSERT INTO "GovernedMeetingRsvp" ("id","meetingParticipantId","status","version","meetingRevision","createdAt","updatedAt") VALUES (gen_random_uuid()::text,'missing-participant','PENDING',0,1,now(),now()); ELSIF kind='bad_user' THEN INSERT INTO "GovernedMeetingRsvp" ("id","meetingParticipantId","status","decidedAt","decidedByUserId","version","meetingRevision","createdAt","updatedAt") VALUES (gen_random_uuid()::text,participant_id,'ACCEPTED',now(),'missing-user',0,1,now(),now()); END IF; RETURN false; EXCEPTION WHEN OTHERS THEN RETURN true; END; END $fn$;`);
    for (const kind of ["pending_decided", "accepted_no_date", "declined_no_date", "duplicate", "bad_revision", "bad_participant", "bad_user"]) {
      const [row] = await tx.$queryRawUnsafe(`SELECT pg_temp.rsvp_invalid($1,$2,$3) AS blocked`, kind, participant.id, template.workspace.ownerId);
      observed[kind] = row?.blocked === true;
    }
