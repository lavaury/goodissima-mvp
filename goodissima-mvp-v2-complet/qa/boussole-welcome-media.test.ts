...
test("welcome discovery keeps a single text column without video", () => {
  assert.equal(existsSync(new URL("../components/BoussoleWelcomeVideo.tsx", import.meta.url)), false);
  assert.equal(existsSync(new URL("../public/media/boussole-introduction.mp4", import.meta.url)), false);
  assert.doesNotMatch(page, /BoussoleWelcomeVideo|<video|grid-cols/);
  assert.match(page, /<div className="min-w-0">/);
});

test("keeps the welcome header independent from Boussole state and business actions", () => {
  assert.doesNotMatch(page, /onPlay=|onEnded=|onTimeUpdate=|onLoadedMetadata=|fetch\s*\(|router\.(?:push|replace)|useRouter/);
  assert.doesNotMatch(page, /WELCOME_STEP_IDS|WELCOME_MODES|welcomeEntries|matching|journeyVersion|data-boussole-id/i);
